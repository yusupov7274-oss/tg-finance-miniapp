...
function App() {
  ...
  // Автоматическая авторизация через Telegram WebApp initData при загрузке
  useEffect(() => {
      ...
      <div className="app-content">
        <div className="auth-block" style={{ marginBottom: '12px', padding: '10px 12px', background: 'var(--bg-secondary, #1e1e1e)', borderRadius: '8px', fontSize: '14px' }}>
          {authStatus === 'no-telegram' && (
            <span style={{ color: '#b0b0b0' }}>Откройте приложение внутри Telegram</span>
          )}
          {authStatus === 'loading' && (
            <span style={{ color: '#b0b0b0' }}>⏳ Авторизация...</span>
          )}
          {authStatus?.ok === true && authStatus.user && (
            <span style={{ color: '#7cb342' }}>✅ Авторизован: {authStatus.user.first_name || '—'}</span>
          )}
          {authStatus && authStatus.ok === false && (
            <span style={{ color: '#f44336' }}>❌ Ошибка авторизации</span>
          )}
        </div>
        {activeTab === 'dashboard' && (
          <Dashboard 
